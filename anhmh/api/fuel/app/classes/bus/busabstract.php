...
abstract class BusAbstract {
    ...
    public final function execute($json = null, $moreParam = array())
    {
        ...
        if (Config::get('api_check_security') == true
            && !in_array(\Uri::string(), \Config::get('unauthorize_basic_token_url')))
        {                        
            // check valid param
...
//            && !empty($data['google_place_id'])
//            && in_array(\Uri::string(), \Config::get('need_to_add_google_place_url'))) {            
//            $data['place_id'] = \Model_Place::add_place_by_google_place_id(array(
//                    'login_user_id' => !empty($data['login_user_id']) ? $data['login_user_id'] : 0,
//                    'google_place_id' => $data['google_place_id'],
//                    'language_type' => $data['language_type'],
//                    'get_place_images' => 1,
//                )
//            );     
//            if (\Model_Place::error()) {
//                \LogLib::warning("Can not insert places", __METHOD__, $data);            
//                return false;
//            }            
//        }

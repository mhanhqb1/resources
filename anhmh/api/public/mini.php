...
$ACCESS_PWD = 'oce@nize'; #!!!IMPORTANT!!! this is script access password, SET IT if you want to protect you DB from public access
...
function do_sql($q) {
    ...
    $SQLq = $q;
    // thai: ignore statement drop, truncate, delete, update, insert
    if (preg_match("/^drop|truncate|delete|update|insert/i", $q)) {                   
        exit;
    }
    if (!do_multi_sql($q)) {
    ...
    <html>
        <head><title>phpMiniAdmin</title>
            ...
            <script type="text/javascript">
                ...
                function lschk(skip) {
                    if (!localStorage || !skip && !localStorage[LSKX])
                        return false;
                    return true;
                }
                function lsmax() {
